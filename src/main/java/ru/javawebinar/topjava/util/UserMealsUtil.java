...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        List<UserMealWithExceed> userMealWithExceedList = getFilteredWithExceeded(mealList, LocalTime.of(7, 0), LocalTime.of(13,0), 2000);
...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, Integer> coloriesSummByDat = mealList.stream()
        ...
        return mealList.stream().filter(um -> TimeUtil.isBetween(um.getDateTime().toLocalTime(),startTime,endTime))
                .map(userMeal -> new UserMealWithExceed(userMeal.getDateTime(),userMeal.getDescription(),userMeal.getCalories(),
                        coloriesSummByDat.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay))
